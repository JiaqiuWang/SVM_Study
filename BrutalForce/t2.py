...
class SupportVectorMachine:
    ...
    def visualize(self):
        data = [[self.ax.scatter(x[0], x[1], s=100, color=self.colors[i]) for x in data_dict[i]] for i in data_dict]
        print("data:\n", data, ", type: ", type(data))

        def hyperplane(x, w, b, v):
            print("w[0]:", w[0], ", x:", x, ", b:", b, ", v(y):", v, ", y值：",
                  (-w[0] * x - b + v) / w[1])
            # 按w1*x + w2*y + b = v的方程进行推到、计算
            return (-w[0] * x - b + v) / w[1]

        datarage = (self.min_feature_value, self.max_feature_value)
        print("datarage:", datarage)

        hyp_x_min = datarage[0]
        hyp_x_max = datarage[1]
        print("min:", hyp_x_min, ", max:", hyp_x_max)

        psv1 = hyperplane(hyp_x_min, self.w, self.b, 1)  # y=1值
        psv2 = hyperplane(hyp_x_max, self.w, self.b, 1)
        p1, = self.ax.plot([hyp_x_min, hyp_x_max], [psv1, psv2], 'g')

        nsv1 = hyperplane(hyp_x_min, self.w, self.b, -1)
        nsv2 = hyperplane(hyp_x_max, self.w, self.b, -1)
        p2, = self.ax.plot([hyp_x_min, hyp_x_max], [nsv1, nsv2], 'k')

        db1 = hyperplane(hyp_x_min, self.w, self.b, 0)
        db2 = hyperplane(hyp_x_max, self.w, self.b, 0)
        p3, = self.ax.plot([hyp_x_min, hyp_x_max], [db1, db2], 'y--')

        plt.legend([p3, p2, p1], ["wx+b=0", "wx+b=-1", "wx+b=1"], loc='best')
        plt.show()
